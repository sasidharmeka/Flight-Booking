...
def main(env,gcs_bucket,bq_project, bq_dataset, transformed_table, route_insights_table, origin_insights_table,route_summarization_table,sales_summarization_table):
    try:
        ...
        input_path = f"gs://{gcs_bucket}/source-{env}"
        ...
        route_summarization_insights = transformed_data.groupBy("route").agg(
            count("*").alias("total_bookings"),
            avg("purchase_lead").alias("avg_purchase_lead"),
            avg("is_weekend").alias("weekend_ratio")
        )
        sales_summarization_insights = transformed_data.groupBy("sales_channel").agg(
            count("*").alias("total_bookings"),
            avg("purchase_lead").alias("avg_purchase_lead"),
            avg("is_weekend").alias("weekend_ratio")
        )
        ...
        booking_origin_insights.write \
         ...
         # Write booking route summarization insights to BigQuery
        logger.info(f"Writing booking route summarization insights to BigQuery table: {bq_project}:{bq_dataset}.{route_summarization_table}")
        route_summarization_insights.write \
            .format("bigquery") \
            .option("table", f"{bq_project}:{bq_dataset}.{route_summarization_table}") \
            .option("writeMethod", "direct") \
            .mode("overwrite") \
            .save()
        
         # Write sales channel summarization insights to BigQuery
        logger.info(f"Writing sales channel summarization insights to BigQuery table: {bq_project}:{bq_dataset}.{sales_summarization_table}")
        sales_summarization_insights.write \
            .format("bigquery") \
            .option("table", f"{bq_project}:{bq_dataset}.{route_summarization_table}") \
            .option("writeMethod", "direct") \
            .mode("overwrite") \
...
if __name__ == "__main__":
    ...
    parser.add_argument("--route_summarization_table", required=True, help="BigQuery table for route summarization insights")
    parser.add_argument("--sales_summarization_table", required=True, help="BigQuery table for sales summarization insights")
    ...
    main(
        ...
        route_summarization_table=args.route_summarization_table,
        sales_summarization_table=args.sales_summarization_table
    )